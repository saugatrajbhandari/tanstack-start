...
import { checkDeploymentPresetInput, getUserViteConfig, inlineConfigSchema, serverSchema, } from './schema.js';
...
export async function defineConfig(inlineConfig = {}) {
    ...
    const globalMiddlewareEntry = opts.routers?.server?.globalMiddlewareEntry ||
    ...
    const TanStackServerFnsPlugin = createTanStackServerFnPlugin({
        ...
        client: {
            getRuntimeCode: () => `import { createClientRpc } from '@tanstack/react-start/server-functions-client'`,
            replacer: (opts) => `createClientRpc('${opts.functionId}', '${serverBase}')`,
        },
        ssr: {
            getRuntimeCode: () => `import { createSsrRpc } from '@tanstack/react-start/server-functions-ssr'`,
            replacer: (opts) => `createSsrRpc('${opts.functionId}', '${serverBase}')`,
        },
        server: {
            getRuntimeCode: () => `import { createServerRpc } from '@tanstack/react-start/server-functions-server'`,
        ...
        routers: [
            ...
            },
            {
                ...
                plugins: () => {
                    ...
                    const clientViteConfig = getUserViteConfig(opts.routers?.[routerType]?.vite);
                    return [
            ...
            },
            {
                ...
                plugins: () => {
                    ...
                    const ssrViteConfig = getUserViteConfig(opts.routers?.[routerType]?.vite);
                    return [
            ...
            },
            {
                ...
                // TODO: RSCS - enable this
                // worker: true,
                handler: importToProjectRelative('@tanstack/start-server-functions-handler'),
                plugins: () => {
                    const routerType = 'server';
                    const serverViteConfig = getUserViteConfig(opts.routers?.[routerType]?.vite);
                    return [
    ...
    vinxiApp.hooks.hook('app:dev:nitro:config', (devServer) => {
        vinxiApp.hooks.hook('app:dev:server:created', ({ devApp: { localFetch } }) => {
